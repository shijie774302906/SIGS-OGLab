...
export function sha256HexSync(value: string) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const t1 = (h + sigma1 + choice + SHA256_CONSTANTS[index] + words[index]) >>> 0;
...
function stringifyStableValue(value: unknown): string | undefined {
  if (value === null) return 'null';
  if (Array.isArray(value)) {
    return `[${value.map((item) => stringifyStableValue(item) ?? 'null').join(',')}]`;
  }
  switch (typeof value) {
    case 'string':
    case 'number':
    ...
    case 'object': {
      ...
      const properties = Object.keys(record)
        ...
        .flatMap((key) => {
          const nested = stringifyStableValue(record[key]);
          return nested === undefined ? [] : [`${JSON.stringify(key)}:${nested}`];
        });
