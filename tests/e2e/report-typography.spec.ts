...
test('PROCESS151 report typography keeps one physical point-size contract across quick and professional pages', () => {
  expect(REPORT_FONT_PT_FLOORS).toEqual({ source: 8, legend: 9, body: 10, title: 12 });
  expect(QUICK_REPORT_FONT_PT_FLOORS).toEqual(REPORT_FONT_PT_FLOORS);

  expect({
    portraitLegend: quickReportLogicalPixelsForPoints('portrait', 9),
    portraitBody: quickReportLogicalPixelsForPoints('portrait', 10),
    portraitTitle: quickReportLogicalPixelsForPoints('portrait', 12),
    landscapeLegend: quickReportLogicalPixelsForPoints('landscape', 9),
  ...
  expect(reportResolvedFontSize('portrait', 1400, 6, 'legend', 595.28)).toBe(22);
