...
test('PROCESS151 professional output renders A4 and A3 pages without falling below the physical source-text floor', async ({ page }) => {
  const rendering = await page.evaluate(async () => {
    ...
    const layers = [
      ...
      { layerId: 'layer-2', name: '粉土—砂土过渡层 2', depthFromM: 5, depthToM: 11, engineeringSoilGroup: 'mixed' },
  ...
  if (process.env.MILESTONE_EVIDENCE === '1') {
    const evidenceDirectory = path.join(process.cwd(), 'process_logs', 'playwright-mcp', 'process151-professional-typography');
    mkdirSync(evidenceDirectory, { recursive: true });
    const saveDataUrl = (name: string, dataUrl: string) => writeFileSync(
      path.join(evidenceDirectory, name),
      Buffer.from(dataUrl.slice(dataUrl.indexOf(',') + 1), 'base64'),
    );
    saveDataUrl('professional-a4-evidence-page.png', rendering.a4[1]);
    saveDataUrl('professional-a3-depth-page.png', rendering.a3[1]);
    saveDataUrl('professional-a3-parameter-page.png', rendering.a3[Math.min(3, rendering.a3.length - 1)]);
    writeFileSync(path.join(evidenceDirectory, 'professional-font-check.json'), JSON.stringify({
      ...
      physicalFloorsPt: { source: 8, legend: 9, body: 10, title: 12 },
    }, null, 2));
  }
});
